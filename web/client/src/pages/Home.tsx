...
export default function Home() {
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      <div className="brand"><img src="/assets/clock-signal-mark.png" alt="Premium Clock mark" className="brand-mark" /><div className="brand-lockup"><span className="brand-name">PREMIUM:TIME</span><span className="brand-subline">Local instrument</span></div></div>
    ...
    <main className="workspace">
      <header className="topbar"><div><p className="eyebrow">Instrument view</p><div className="topbar-title">{nav.find((item) => item.id === view)?.label}<span className="topbar-index">// 01</span></div></div><div className="topbar-actions"><a className="outline-button apk-download" href="https://github.com/spdedsec/premium-clock/releases/download/v1.0.0/premium-clock-android-v1.0.0-debug.apk" download><Download className="button-icon" /> Android APK</a><button type="button" className="outline-button search-control" onClick={() => setSearchOpen(true)}><Search className="button-icon" /><span className="utility-label">Search</span><span className="foot-shortcut">⌘ K</span></button><button type="button" className="icon-button" aria-label="Toggle dark mode" onClick={() => setSettings((current) => ({ ...current, theme: current.theme === "dark" ? "light" : "dark" }))}>{settings.theme === "dark" ? <Sun className="button-icon" /> : <Moon className="button-icon" />}</button></div></header>
